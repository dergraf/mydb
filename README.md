...
<code>
%% PUT Request    
    mydb:put(Key, Value) -> ok.                 %% Key :: term(), Value :: term()
    mydb:put(Key, Value) -> ok.                 %% Key :: binary(), Value :: binary()

%% GET Request
    mydb:get(Key) -> Value.                     %% Key :: term(), Value :: term()
    mydb:getb(Key) -> Value.                    %% Key :: binary(), Value :: binary()

%% UPDATE Request
    mydb:update(Key, Value) -> ok.              %% Key :: term(), Value :: term()
    mydb:updateb(Key, Value) -> ok.             %% Key :: binary(), Value :: binary()

%% DELETE Request
    mydb:delete(Key) -> ok.                     %% Key :: term()
    mydb:delete(Key) -> ok.                     %% Key :: binary()

%% ADD MEMBERSHIP
    mydb:add_membership(Group, Key) -> ok.      %% Group :: term(), Key :: term()

%% GET MEMBERS
    mydb:get_members(Group) -> [{Key, Value}].  %% Group :: term(), Key :: term(), Value :: term()
</code>
...
Basho Benchmark
---------------
Check summary.png to see the results of a bashobench run on my Macbook Pro 2GHz Intel Code i7, SATA HD with 7200 RPM
